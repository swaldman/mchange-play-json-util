...
object BufferedJsValue {
  def apply( arr : Array[Byte] ) : BufferedJsValue = this.apply( ImmutableArraySeq.apply[Byte](arr) ) // this could be from anywhere, better copy it
...
case class BufferedJsValue( buffer : Seq[Byte] ) {
  def toJsValue : JsValue = Json.parse( buffer.toArray )

  def transform( transformer : Seq[Byte] => Seq[Byte] ) : BufferedJsValue = BufferedJsValue( transformer( buffer ) )
}
